require 'colorizr'
String.create_color

require_relative "game"
require_relative "tribe"
require_relative "contestant"
require_relative "jury"

#After your tests pass, uncomment this code below
#=========================================================
# # Create an array of twenty hopefuls to compete on the island of Borneo
@contestants = %w(carlos walter aparna trinh diego juliana poornima juha sofia julia fernando dena orit colt zhalisa farrin muhammed ari rasha gauri)
@contestants.map!{ |contestant| Contestant.new(contestant) }.shuffle!
#
# # Create two new tribes with names
@coyopa = Tribe.new(name: "Pagong", members: @contestants.shift(10))
@hunapu = Tribe.new(name: "Tagi", members: @contestants.shift(10))
#
# # Create a new game of Survivor
@borneo = Game.new(@coyopa, @hunapu)
#=========================================================

PHASE_ONE_ELIMINATION_ROUNDS = 8
PHASE_TWO_ELIMINATION_ROUNDS = 3
PHASE_THREE_ELIMINATION_ROUNDS = 7

#This is where you will write your code for the three phases
def phase_one
  puts " *** Phase 1 starts ***".red
  PHASE_ONE_ELIMINATION_ROUNDS.times do
    losing_team = @borneo.immunity_challenge
    puts "#{losing_team.to_s.white} is lost in immunity challenge."
    voted_off_contestant = losing_team.tribal_council
    puts "#{voted_off_contestant.name.capitalize.pink} has been eliminated in tribal council."
  end
end

def phase_two
  puts " *** Phase 2 starts ***".red
  PHASE_TWO_ELIMINATION_ROUNDS.times do
    voted_off_contestant = @merge_tribe.tribal_council
    puts "#{voted_off_contestant.name.capitalize.pink} has been eliminated in tribal council."
  end
end

def phase_three
  puts " *** Phase 3 starts ***".red
  PHASE_THREE_ELIMINATION_ROUNDS.times do
    voted_off_contestant = @merge_tribe.tribal_council
    @jury.add_member(voted_off_contestant)
    puts "#{voted_off_contestant.name.capitalize.pink} has been eliminated in tribal council and becomes a jury."
  end
end


# If all the tests pass, the code below should run the entire simulation!!
#=========================================================
phase_one # 8 eliminations
@merge_tribe = @borneo.merge("Cello") # After 8 eliminations, merge the two tribes together
puts "The teams have been merged into " + @merge_tribe.to_s.upcase.white
phase_two # 3 more eliminations
@jury = Jury.new
phase_three # 7 elminiations become jury members
finalists = @merge_tribe.members # set finalists
vote_results = @jury.cast_votes(finalists) # Jury members report votes
@jury.report_votes(vote_results) # Jury announces their votes
@jury.announce_winner(vote_results) # Jury announces final winner
